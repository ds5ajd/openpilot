...
class CarInterface(CarInterfaceBase):
  ...
  def get_pid_accel_limits(CP, current_speed, cruise_speed):
    ...
    accel_max_bp = [10., 20., 30., 50., 70., 80.] #=> [10., 20., 50.]
    accel_max_v =[0.9, 0.95, 0.85, 0.82, 0.87, 0.9] #=> [0.9, 1.0, 0.85, 0.82, 0.9, 0.95]
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=None, disable_radar=False):
    ...
    ret.steerRateCost = 0.45 # 0.45 very good 0.35(origin) # def : 2.0
    ret.steerActuatorDelay = 0.00 #0.01  # def: 0.2 Default delay, not measured yet
    ...
    if lateral_control == 'INDI':
      ...
      max_lat_accel = 2.6531724862969748 #2.35
    ...
    ret.longitudinalActuatorDelayUpperBound = 0.39 #0.35(Good)
    
    ret.stopAccel = 0.0 #-2.2
    ret.stoppingDecelRate = 4.0 #4.0
    ret.vEgoStopping = 0.3
    ret.vEgoStarting = 0.2 #0.23(good)
