...
class Controls:
  def __init__(self, sm=None, pm=None, can_sock=None, CI=None):
    ...
    self.sound_eventfired = False

    if not sounds_available and not self.sound_eventfired :
      # sounds_available = True
      self.sound_eventfired = True
      self.events.add(EventName.soundsUnavailableOnce, static=False)
  ...
  def state_control(self, CS):
    ...
    if not self.joystick_mode:
      ...
      actuators.accel = self.LoC.update(CC.longActive and CS.cruiseState.enabledAcc, CS, self.CP, long_plan, pid_accel_limits, t_since_plan,
                                        self.sm['radarState'])
    ...
    if lac_log.active and lac_log.saturated and not CS.steeringPressed:
      ...
      if len(dpath_points):
        ...
        left_deviation = actuators.steer > 0 and dpath_points[0] < -0.11 #-0.35
        right_deviation = actuators.steer < 0 and dpath_points[0] > 0.11 #0.35
